...
allearter %>% dplyr::filter(!Orden %in% remove) %>% group_by(R�dlistestatus) %>% dplyr::summarise(value = n())
...
redlist <- allearter %>% dplyr::select(`Videnskabeligt navn`, R�dlistestatus, Fredningsstatus, `Bonn-konventionen`, `Bern-konventionen`, CITES, `NOBANIS-arter`, `NOBANIS (herkomst)`, `NOBANIS (etableringsstatus)`, `NOBANIS (invasiv optr�den)`, `�vrige forvaltningskategorier`)

redlistcar <- merge(unique_species, redlist, by.x = "species", by.y = "Videnskabeligt navn")
table(redlistcar$R�dlistestatus)
...
speciesest <- test2 %>% dplyr::rename(sample = `N.obs`, Observed = S.obs, s.obs.upper = `S.obs(+95%)`, s.obs.lower = `S.obs(-95%)`,  chao1.upper = `Chao1(upper)`, chao1.lower = `Chao1(lower)`, ACE.upper = `ACE(upper)`, ACE.lower = `ACE(lower)`, jack1.upper = `Jack1(upper)`, jack1.lower = `Jack1(lower)`)
...
data <- merge(data, richnessdata, by = "SampleID")
...
### mean richness per administrative region ####
...
# Error bars represent standard error of the mean (not in MS)
tgc %>% ggplot(mapping = aes(x=reorder(regionNavn,-order_richness), y=order_richness, fill = order_final)) + geom_bar(stat="identity", position = position_dodge(), show.legend = T)  +
...
### Total richness per region & chao1 estimate ##############

dat <- ddply(data, .(regionNavn), summarise, mean_chao = mean(Chao1))

dat = within(dat, {
  region = factor(regionNavn)
  mean_chao_string = sprintf('%0.1f', mean_chao)
})

ggplot(dat, aes(x = region, y = mean_chao)) + 
  geom_bar(aes(fill = region), stat = "identity") + 
  geom_text(aes(label = mean_chao_string), vjust = -0.5)

plot_region <-  data %>% ggplot(mapping = aes(x=reorder(regionNavn,-richness), y=richness, fill = regionNavn, label = mean_chao_string)) + geom_bar(stat="identity", width=0.5, position = "dodge", show.legend = F) + theme_cleveland() +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank(), axis.title.x = element_text(size = 12, face = "bold"),  axis.title.y = element_text(size = 12, face = "bold"), panel.background = element_rect(fill = "white", colour = "white", size = 0.5, linetype = "solid")) + scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(limits = c(0, 100, 200, 300, 400), expand = c(0, 0)) + scale_fill_manual(values = wes_palette(name = "Moonrise3")) + labs(
          ...
          fill = "Biogeographic region \nin Denmark") # chao1 has to be added manually
  
data <- merge(data, environ, by = "SampleID")
data[ ,c(35:58, 79:146)] <- data[,c(35:58, 79:146)]*100 # scale by 100 to get percentage cover
...
dat <- ddply(data, .(hab50), summarise, mean_chao = mean(Chao1))

dat = within(dat, {
  landcover = factor(hab50)
  mean_chao_string = sprintf('%0.1f', mean_chao)
})

ggplot(dat, aes(x = landcover, y = mean_chao)) + 
  geom_bar(aes(fill = landcover), stat = "identity") + 
  geom_text(aes(label = mean_chao_string), vjust = -0.5)

plot_cover <- data %>% mutate(hab50 = fct_relevel(hab50, "Farmland50", "Forest50", "Mix", "Urban50")) %>% ggplot(aes(
  x = hab50,
  y = richness,
  fill = hab50,
  group = 1
  ...
  legend.position = "bottom"
) + scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(limits = c(0, 100, 200, 300, 400, 500), expand = c(0, 0)) + scale_fill_manual(
...
data_syr <- data_syr[, c(1:20, 36:149)] # remove old diversity estimates

# rarefy using phyloseq
t_asvs <- as.data.frame(tasvs)
tasv_syr <- t_asvs[, (names(t_asvs) %in% syrphids)]
asv_syr <- t(tasv_syr)
asv_syr <- as.data.frame(asv_syr)
rowSums(asv_syr)
colSums(asv_syr)
asv_syr <- asv_syr[, colSums(asv_syr) > 0]
OTU = otu_table(asv_syr, taxa_are_rows = TRUE)
rarified <- rarefy_even_depth(OTU, sample.size = min(sample_sums(OTU)), replace = FALSE, trimOTUs = FALSE, rngseed = 711)
rarefied.data = t(as(otu_table(rarified), "matrix")) 
rarefied.data <- t(rarefied.data)
rarefied.data <- as.data.frame(rarefied.data)
...
data_syr <- cbind(data_syr, speciesest)

dat <- ddply(data_syr, .(regionNavn), summarise, mean_chao = mean(Chao1))
...
dat <- ddply(data_syr, .(hab50), summarise, mean_chao = mean(Chao1))
...
### all insects ########
data_sub <- data %>% dplyr::filter(!regionNavn == "Bornholm")

hist(data_sub$richness)
hist(log(data_sub$richness))
hist(sqrt(data_sub$richness))

hist(data_sub$Chao1)
hist(log(data_sub$Chao1))
hist(sqrt(data_sub$Chao1))
...
lmeregion <- lmer(sqrt(richness) ~ 
                  regionNavn + 
                  Wind +
                  (1|RouteID) + (1|PID), data=data_sub)

summary(lmeregion)
qqnorm(resid(lmeregion))
AICc(lmeregion)
...
lmecover <- lmer(sqrt(richness) ~ 
                    hab50 + 
                    #Wind +
                    (1|RouteID) + (1|PID), data=data_sub)

summary(lmecover)
qqnorm(resid(lmecover))
AICc(lmecover)

tab_model(lmeregion, pred.labels = c("Intercept (Funen & Gentle breeze)", "West Jutland", "East Jutland", "Sealand", "Light breeze", "Moderate breeze"))

tab_model(lmecover, pred.labels = c("Intercept (>50% Farmland cover)", ">50% Forest cover", "Mixed cover", ">50% Urban cover"))

# emmeans
ems <- emmeans(lmeregion, specs = pairwise ~ regionNavn, type = "response")
ems$contrasts
ems$emmeans
plot(ems, comparisons = TRUE)

ems <- emmeans(lmecover, specs = pairwise ~ hab50, type = "response")
ems$contrasts
ems$emmeans
plot(ems, comparisons = TRUE)

# diversity
regionchao1 <- lmer(Chao1 ~ 
                  regionNavn + 
                  (1|RouteID) + (1|PID), data=data_sub)

summary(regionchao1)
qqnorm(resid(regionchao1))
AICc(regionchao1)

tab_model(regionchao1, pred.labels = c("Intercept (Funen)", "West Jutland", "East Jutland", "Sealand")) # very much the random effect explaining the variation

# emmeans
#pairs(emmeans(regionchao1, ~regionNavn))
ems <- emmeans(regionchao1, specs = pairwise ~ regionNavn, type = "response")
ems$contrasts
ems$emmeans
plot(ems)

landcoverchao1 <- lmer(Chao1 ~ 
                  hab50 + 
                  (1|RouteID) + (1|PID), data=data_sub)

summary(landcoverchao1)
qqnorm(resid(landcoverchao1))
AICc(landcoverchao1)

tab_model(landcoverchao1, pred.labels = c("Intercept (Farmland)", "Forest", "Mix", "Urban")) # very much the random effect explaining the variation

ems <- emmeans(landcoverchao1, specs = pairwise ~ hab50, type = "response")
ems$contrasts
ems$emmeans
plot(ems, comparisons = TRUE)

### syrphidae richness #########
qqnorm(data_syr$syr_richness)
hist(log(data_syr$syr_richness))
qqnorm(sqrt(data_syr$syr_richness)) # not normally distributed by transformation

table(data_syr$regionNavn)
data_syr_sub <- data_syr %>% dplyr::filter(!regionNavn == "Bornholm")

lmesyrregion <- lmer(sqrt(syr_richness) ~ 
                  regionNavn + 
                  #hab50 +
                  #Wind +
                  #Temperature +
                  (1|RouteID) + (1|PID), data=data_syr_sub)

summary(lmesyrregion)
qqnorm(resid(lmesyrregion))
AICc(lmesyrregion)

lmesyrcover <- lmer(sqrt(syr_richness) ~ 
                  #regionNavn + 
                  hab50 +
                  #Wind +
                  #Temperature +
...
summary(lmesyrcover)
qqnorm(resid(lmesyrcover))
AICc(lmesyrcover)

tab_model(lmesyrregion, pred.labels = c("Intercept (Funen)", "West Jutland", "East Jutland", "Sealand"))

tab_model(lmesyrcover, pred.labels = c("Intercept (>50% farmland cover)", ">50% forest cover", "Mixed cover", ">50% urban cover"))

ems <- emmeans(lmesyrregion, specs = pairwise ~ regionNavn, type = "response")
ems$contrasts
ems$emmeans
plot(ems, comparisons = TRUE)

ems <- emmeans(lmesyrcover, specs = pairwise ~ hab50, type = "response")
ems$contrasts
ems$emmeans
plot(ems, comparisons = TRUE)
